...
public interface PhoneDataRepository extends JpaRepository<PhoneDataDao, Long> {
    ...
    @Query(nativeQuery = true, value="SELECT * FROM phone_data WHERE phone = :phoneNumber AND user_id = :userId")
    Optional<PhoneDataDao> findByPhoneNumberAndUserId(String phoneNumber, Long userId);
}
